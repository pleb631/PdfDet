...
def ap_per_class(
    ...
    px, py = np.linspace(0, 1, 2000), []  # for plotting
    ap, p, r = np.zeros((nc, tp.shape[1])), np.zeros((nc, 2000)), np.zeros((nc, 2000))
...
class DetMetrics:
    ...
    def __init__(
        self, save_dir=Path("."), plot=False, on_plot=None, names=(), **kwargs
    ) -> None:
        self.save_dir = Path(save_dir)
        self.plot = plot
        self.on_plot = on_plot
        if isinstance(names,list):
            names = {i:j for i,j in enumerate(names)}
        self.names = names
        self.box = Metric()
        self.speed = {
            "preprocess": 0.0,
